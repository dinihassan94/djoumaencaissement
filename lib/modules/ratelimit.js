module.exports = {
  ...
  consume: function (options) {
    ...
    if (options.instance) {
      ...
      return rateLimiter.consume(req.ip, options.points).catch(() => {
        if (options.throw) {
      ...
      app[isPrivate ? 'privateRateLimiter' : 'rateLimiter'].consume(key, points).then((rateLimiterRes) => {
        ...
        res.set('RateLimit-Policy', `${limit};w=${duration}`);
        res.set('RateLimit', `limit=${limit}, remaining=${rateLimiterRes.remainingPoints}, reset=${reset}`);
        next();
        ...
        if (options.throw) {
          ...
          if (req.is('json')) {
            res.status(options.status).json({ error: options.message });
          } else {
